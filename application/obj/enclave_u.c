...
static sgx_status_t SGX_CDECL enclave_ocall_save_wallet(void* pms)
{
	...
	ms->ms_retval = ocall_save_wallet(ms->ms_data, ms->ms_size);
...
static sgx_status_t SGX_CDECL enclave_ocall_load_wallet(void* pms)
{
	...
	ms->ms_retval = ocall_load_wallet(ms->ms_data, ms->ms_size);
...
{
	sgx_status_t status;
	ms_ecall_print_prime_random_t ms;
...
sgx_status_t ecall_add_item(sgx_enclave_id_t eid, int* retval, const char* master_password, const char* title, const char* username, const char* password)
{
	...
	ms.ms_master_password_len = master_password ? strlen(master_password) + 1 : 0;
	ms.ms_title = title;
	ms.ms_title_len = title ? strlen(title) + 1 : 0;
